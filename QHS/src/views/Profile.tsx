...
import axiosClient, { assetUrl, getApiErrorMessage } from '../axiosClient';
...
type PasswordFieldName = keyof PasswordData;
...
export default function Profile() {
  ...
  const handleProfileUpdate = async (event: FormEvent) => {
    ...
    try {
      ...
      const response = await axiosClient.post<{ user: AppUser }>('/profile/update', payload, { headers: { 'Content-Type': 'multipart/form-data' } });
      setUser(response.data.user);
      setProfileImage(null);
      setSuccessMessage('Your profile details were updated.');
    } catch (error: unknown) {
      setErrorMessage(getApiErrorMessage(error, 'Your profile could not be updated.'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handlePasswordUpdate = async (event: FormEvent) => {
    ...
    try {
      ...
      setErrorMessage(getApiErrorMessage(error, 'Your password could not be updated.'));
    } finally {
      setLoading(false);
    }
  };

  const passwordField = (label: string, name: PasswordFieldName, key: PasswordVisibilityKey, autoComplete: string) => (
    <TextField
      fullWidth
      required
      label={label}
