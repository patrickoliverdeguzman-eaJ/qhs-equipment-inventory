...
import type { ApiErrors, AppUser, UserRole } from '../../types/domain';
...
export default function Users() {
  ...
  const [sortConfig, setSortConfig] = useState<{ key: keyof AppUser | null; direction: 'asc' | 'desc' }>({ key: null, direction: 'asc' });
  ...
  const onDeleteClick = (user: AppUser) => {
  ...
  const handleAvatarChange = (ev: ChangeEvent<HTMLInputElement>) => {
  ...
  const handleUserFormSubmit = async (ev: FormEvent<HTMLFormElement>) => {
    ev.preventDefault();
    ...
    try {
      ...
      const response = axios.isAxiosError<{ errors?: ApiErrors }>(err) ? err.response : undefined;
  ...
  const toggleActiveStatus = (user: AppUser) => {
    const updatedStatus = !Boolean(user.isActive);
    axiosClient
  ...
  const requestSort = (key: keyof AppUser) => {
    ...
    <Box>
      ...
      <Dialog open={openDeleteDialog} onClose={handleCloseDeleteDialog}>
        ...
        <DialogActions>
          ...
          <Button onClick={() => selectedUser && onDeleteClick(selectedUser)} color="error" variant="contained" autoFocus>Delete user</Button>
        </DialogActions>
      </Dialog>
