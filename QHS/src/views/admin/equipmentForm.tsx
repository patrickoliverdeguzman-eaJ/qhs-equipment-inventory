...
import type { ApiErrors, Category, Laboratory } from '../../types/domain';
...
interface EquipmentFormRecord extends Omit<EquipmentFormState, 'category_ids' | 'laboratory_id'> {
...
export default function EquipmentForm() {
  ...
  const [equipment, setEquipment] = useState<EquipmentFormState>(emptyEquipment);
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      id ? axiosClient.get<{ data: EquipmentFormRecord }>(`/equipment/${id}`) : Promise.resolve(null),
  ...
  const onSubmit = async (event: FormEvent) => {
    ...
    payload.append('laboratory_id', String(equipment.laboratory_id));
    equipment.category_ids.forEach((categoryId) => payload.append('category_ids[]', String(categoryId)));
    ...
    try {
      ...
      if (axios.isAxiosError<{ errors?: ApiErrors }>(requestError) && requestError.response?.status === 422) setErrors(requestError.response.data.errors || {});
                ...
                <SectionCard>
                  ...
                  <Stack spacing={2.25} sx={{ p: { xs: 2, sm: 2.5 } }}>
                    ...
                    <FormControl fullWidth required error={Boolean(errors.laboratory_id)}>
                      ...
                      <Select labelId="equipment-lab-label" value={equipment.laboratory_id} label="Laboratory" onChange={(event) => setEquipment((current) => ({ ...current, laboratory_id: event.target.value === '' ? '' : Number(event.target.value) }))}>
                    ...
                    <FormControl fullWidth error={Boolean(errors.category_ids)}>
                      ...
                      <Select<number[]> labelId="equipment-category-label" multiple value={equipment.category_ids} label="Categories" onChange={(event) => setEquipment((current) => ({ ...current, category_ids: typeof event.target.value === 'string' ? event.target.value.split(',').map(Number) : event.target.value }))} renderValue={(selected) => <Stack direction="row" spacing={0.5} useFlexGap flexWrap="wrap">{selected.map((categoryId) => <Chip key={categoryId} label={categories.find((category) => category.id === categoryId)?.name || categoryId} size="small" />)}</Stack>}>
