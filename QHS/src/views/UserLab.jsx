...
export default function UserLab() {
      ...
      try {
        ...
        equipmentRows.forEach((record) => {
          ...
          const available = units.filter((item) => {
            const notBorrowed = item.isBorrowed === 'false' || item.isBorrowed === false;
  ...
  const getAvailableCount = (record) => equipmentItemCounts[record.id]?.available || 0;
  const getLabName = (labId) => laboratories.find((lab) => lab.id === labId)?.name || 'Unassigned laboratory';
  const getImageSrc = (imagePath) => imagePath ? `${backendBaseUrl}/storage/${imagePath}` : assetUrl(null);

  const addQuantityToCart = (record, amount = 1) => {
        ...
        <Grid container spacing={2} alignItems="center" sx={{ p: { xs: 2, sm: 2.5 } }}>
          ...
          <Grid item xs={12} sm={7} md={4}>
            <FormControl fullWidth>
              ...
              <Select labelId="laboratory-filter-label" value={selectedLab} label="Laboratory" onChange={(event) => setSelectedLab(event.target.value)}>
              ...
              <Grid item xs={12} sm={6} md={4} lg={3} key={record.id}>
                <Card variant="outlined" sx={{ height: '100%', display: 'flex', flexDirection: 'column', '&:hover': { borderColor: 'primary.light' } }}>
                  <Box sx={{ position: 'relative', height: 190, overflow: 'hidden', bgcolor: 'action.hover' }}>
                    <Box component="img" src={getImageSrc(record.image)} alt={record.name} onError={(event) => { event.currentTarget.src = assetUrl(null); }} sx={{ width: '100%', height: '100%', objectFit: 'cover' }} />
            ...
            <DialogContent dividers>
              <Grid container spacing={2.5}>
                <Grid item xs={12} sm={5}>
                  <Box component="img" src={getImageSrc(selectedEquipment.image)} alt={selectedEquipment.name} onError={(event) => { event.currentTarget.src = assetUrl(null); }} sx={{ width: '100%', aspectRatio: '1 / 1', objectFit: 'cover', borderRadius: 2, bgcolor: 'action.hover' }} />
                </Grid>
